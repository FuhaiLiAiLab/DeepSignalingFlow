...
from enc_dec.geo_bgnn_decoder import BGNNDecoder
...
def build_geoBgnn_model(args, device, dataset):
    print('--- BUILDING UP BGNN MODEL ... ---')
    # GET PARAMETERS
    ...
    # BUILD UP MODEL
    model = BGNNDecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, 
...
def train_geoBgnn_model(dataset_loader, model, device, args, learning_rate):
...
def train_geoBgnn(args, fold_n, load_path, iteration_num, device, dataset):
    ...
    model = build_geoBgnn_model(args, device, dataset)
    if args.model == 'load':
    ...
    for i in range(1, epoch_num + 1):
        ...
        for index in range(0, dl_input_num, batch_size):
            ...
            model, batch_loss, batch_ypred = train_geoBgnn_model(dataset_loader, model, device, args, learning_rate)
        ...
        # fold_n = 1
        test_save_path = path
        test_pearson, test_loss, tmp_test_input_df = test_geoBgnn(prog_args, fold_n, model, test_save_path, device, dataset)
...
def test_geoBgnn_model(dataset_loader, model, device, args):
    batch_loss = 0
    for batch_idx, data in enumerate(dataset_loader):
...
def test_geoBgnn(args, fold_n, model, test_save_path, device, dataset):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    ...
    for index in range(0, dl_input_num, batch_size):
        ...
        print('TEST MODEL...')
        # import pdb; pdb.set_trace()
        model, batch_loss, batch_ypred = test_geoBgnn_model(dataset_loader, model, device, args)
        print('BATCH LOSS: ', batch_loss)
        batch_loss_list.append(batch_loss)
...
if __name__ == "__main__":
    ...
    train_geoBgnn(prog_args, fold_n, load_path, start_iter_num, device, dataset)
