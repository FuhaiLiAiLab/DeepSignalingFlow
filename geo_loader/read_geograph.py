...
if __name__ == "__main__":
    ...
    x_split1 = np.load('./datainfo/form_data/x_split1.npy')
    y_split1 = np.load('./datainfo/form_data/y_split1.npy')
    drug_split1 = np.load('./datainfo/form_data/drug_split1.npy')
    # READ [edge_index] FILES
    edge_index = np.load('./datainfo/form_data/edge_index.npy')

    # [num_feature, num_gene, num_drug]
    num_feature = 7
    dict_drug_num = pd.read_csv('./datainfo/filtered_data/dict_drug_num.csv')
    num_drug = dict_drug_num.shape[0]
    final_annotation_gene_df = pd.read_csv('./datainfo/filtered_data/final_annotation_gene.csv')
