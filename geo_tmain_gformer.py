...
def arg_parse():
    parser = argparse.ArgumentParser(description='GEO-GFORMER ARGUMENTS.')
...
def build_geogformer_model(args, device):
    print('--- BUILDING UP GFORMER MODEL ... ---')
    # GET PARAMETERS
    ...
    # BUILD UP MODEL
    model = GraphFormerDecoder(input_dim=args.input_dim, hidden_dim=args.hidden_dim, embedding_dim=args.output_dim, decoder_dim=args.decoder_dim,
            num_head=args.num_head, device=device)
...
def train_geogformer_model(dataset_loader, model, device, args, learning_rate):
    ...
    for batch_idx, data in enumerate(dataset_loader):
        ...
        # import pdb; pdb.set_trace()
        ypred = model(x=x, edge_index=edge_index, drug_index=drug_index)
...
def train_geogformer(args, fold_n, load_path, iteration_num, device):
    ...
    model = build_geogformer_model(args, device)
    if args.model == 'load':
    ...
    for i in range(1, epoch_num + 1):
        ...
        # fold_n = 1
        test_save_path = path
        test_pearson, test_loss, tmp_test_input_df = test_geogformer(prog_args, fold_n, model, test_save_path, device)
...
def test_geogformer(args, fold_n, model, test_save_path, device):
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
    print('-------------------------- TEST START --------------------------')
...
if __name__ == "__main__":
    ...
    train_geogformer(prog_args, fold_n, load_path, start_iter_num, device)
