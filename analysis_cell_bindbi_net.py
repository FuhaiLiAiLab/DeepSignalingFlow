...
class NetAnalyse():
    ...
    def plot_target_cell_gene(self, cellline_name, node_threshold):
        ...
        final_rna_df = pd.read_csv('./data/filtered_data/final_rna.csv')
        rna_cellline_array = np.array(final_rna_df[cellline_name])
        log_rna_cellline_array = np.log(rna_cellline_array + 1)
        gene_bind_degree_array = np.array(gene_bind_degree_df['degree'])
        gene_bind_degree_log_rna_array = log_rna_cellline_array * gene_bind_degree_array
        gene_bind_degree_df['log_rna_degree'] = list(gene_bind_degree_log_rna_array)
        gene_bind_degree_df.to_csv('./analysis_nci/cell_gene_weight_bind_degree.csv', index=False, header=True)
        ...
        for bind_row in gene_bind_degree_df.itertuples():
            if bind_row[4] <= node_threshold:
                ...
                bind_node_degree_list.append(bind_row[4])
        ...
        for gene in intersection_list:
            gene_degree = float(gene_bind_degree_df.loc[gene_bind_degree_df['gene_idx'] == gene]['log_rna_degree'])
    ...
    def plot_cell_net2(self, node_threshold, edge_threshold, 
        ...
        for bind_row in gene_bind_degree_df.itertuples():
            if bind_row[4] <= node_threshold:
                ...
                bind_node_name_list.append(bind_row[2])
                bind_node_index_list.append(bind_row[1])
                bind_node_degree_dict[bind_row[1]] = bind_row[4]
        ...
        hilight_nodes = nx.draw_networkx_nodes(bind_filtered_digraph.subgraph(intersection_list), 
                    ...
                    node_size = [float(degree**1.75) / 1.5 for degree in intersection_degree_list],
                    linewidths = 1.0,
                    node_color = 'white'
                    )
        ...
        hilight_drugs = nx.draw_networkx_nodes(bind_filtered_digraph.subgraph(cellline_specific_druglist), 
                    ...
                    node_size = [float(bind_node_degree_dict[drug_idx]**1.75) / 3 for drug_idx in cellline_specific_druglist],
                    linewidths = 1.5,
                    alpha = 1,
                    node_color = 'white',
                    )
        ...
        hilight_drugs = nx.draw_networkx_nodes(bind_filtered_digraph.subgraph(cellline_specific_druglist), 
                    pos = pos,
                    nodelist = cellline_specific_druglist, 
                    node_size = [float(bind_node_degree_dict[drug_idx]**1.75) / 1.0 for drug_idx in cellline_specific_druglist],
                    linewidths = 0.5,
                    alpha = 1,
                    node_color = 'orange',
                    node_shape = 'v'
