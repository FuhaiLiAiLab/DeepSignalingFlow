...
class ReadFile():
    ...
    def nci_combo_input(self):
        ...
        if os.path.exists('../data/init_data') == False:
            os.mkdir('../data/init_data')
        dl_input_df = pd.read_csv('../data/raw_data/NCI60/DeepLearningInput.csv')
        ...
        dl_input_df.to_csv('../data/init_data/almanac_dl_input.csv', index=False, header=True)
    ...
    def parse_drugcomb_fi(self):
        ...
        if os.path.exists('../data/init_data') == False:
            os.mkdir('../data/init_data')
        drugcomb_fi_df = pd.read_csv('../data/raw_data/DrugComb_fi/summary_v_1_5.csv')
        ...
        oneil_comb_syn_new_df.to_csv('../data/init_data/oneil_dl_input.csv', index=False, header=True)

    def gdsc_rnaseq(self):
        ### INTIALIZE [GDSC RNA Sequence Data]
        print('----- READING GDSC RNA Sequence RAW DATA -----')
        rna_df = pd.read_csv('../data/raw_data/GDSC/rnaseq_20191101/rnaseq_fpkm_20191101.csv', low_memory=False)
        rna_df = rna_df.fillna('missing')
        rna_df.to_csv('../data/init_data/gdsc_rnaseq.csv', index=False, header=True)
        print(rna_df.shape)
        # AFTER THIS NEED SOME MANUAL OPERATIONS TO CHANGE COLUMNS AND ROWS NAMES

    def gdsc_cnv(self):
        cnv_df = pd.read_csv('../data/raw_data/GDSC/cnv_20191101/cnv_gistic_20191101.csv', low_memory=False)
        cnv_df = cnv_df.fillna('missing')
        cnv_df.to_csv('../data/init_data/gdsc_cnv.csv', index = False, header = True)
        print(cnv_df.shape)
        # AFTER THIS NEED SOME MANUAL OPERATIONS TO CHANGE COLUMNS AND ROWS NAMES

    def kegg(self):
        kegg_pathway_df = pd.read_csv('../data/raw_data/KEGG/full_kegg_pathway_list.csv')
        ...
        up_kegg_df.to_csv('../data/init_data/up_kegg.csv', index=False, header=True)
        ...
        up_kegg_path_df.to_csv('../data/init_data/up_kegg_path.csv', index=False, header=True)
    ...
    def drugbank(self):
        ...
        drugbank_df = pd.read_table('../data/raw_data/DrugBank/drug_tar_drugBank_all.txt', delimiter='\t')
        ...
        drugbank_df.to_csv('../data/init_data/drugbank.csv', index=False, header=True)
